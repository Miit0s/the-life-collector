...
@export var maximum_percentage: float = 100
...
func _ready() -> void:
	scan_system.change_goal(int(minimum_percentage), int(maximum_percentage))
...
func _process(_delta: float) -> void:
	...
	if object_to_scan.completion_percentage >= minimum_percentage and object_to_scan.completion_percentage <= maximum_percentage:
		scanned_object_is_good.emit()
	else:
		scanned_object_is_bad.emit()
